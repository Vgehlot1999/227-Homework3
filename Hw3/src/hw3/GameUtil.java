...
{  
  ...
  public int getScoreForValue(int value)
  {
	  ...
	  {
		  n = (int) (Math.log10(tilevalue / 3) / Math.log10(2));
  ...
  public TilePosition generateRandomTilePosition(int[][] grid, Random rand, Direction lastMove)
  {
	  ...
	  {
		  pos = new TilePosition (r, grid.length - 1, 0);
		  
		  if (grid[r][grid.length - 1] == 0)
		  {
			  r = rand.nextInt(grid.length);
		  }
		  else
		  {
			  value++;
	  ...
	  {
		  pos = new TilePosition (grid.length - 1, r, 0);
		  
		  if (grid[grid.length - 1][r] == 0)
		  {
			  r = rand.nextInt(grid.length);
		  }
		  else
  ...
  public ArrayList<Move> shiftArray(int[] arr)
  {
	  ...
	  for (int i = 0; i < arr.length - 1; i++)
	  {
		  if (arr[i] == 0)
		  {
			  if (arr[i + 1] != 0)
			  {
				  list.add(new Move(i + 1, i, arr[i + 1]));
				  arr[i] = arr[i + 1];
				  arr[i + 1] = 0;
			  }
		  }
		  else
		  {
			  if(mergeValues(arr[i], arr[i + 1]) != 0)
			  {
				  ...
				  arr[i] = mergeValues(arr[i + 1], arr[i]);
				  arr[i + 1] = 0;
			  }
		  }
	  }
