...
public class Game
{
  ...
  /**
   * sets a temp grid that will be used in the undo method to store previous grid in case player wishes to undo current move
   */
  private Random r;
  /**
  ...
  private GameUtil g;
  /**
   * instance of the GameUtil class
   */
  private int nextVal;
  /**
   * next incoming value that will be the new tile in the game
   */
  private boolean pending;
  /**
   * used mainly in the undo class, is used to check whether or not the player's current move has been finalized or not
   */
  private Direction d;
  /**
   * last direction that was used
   */
  ...
  public Game(int givenSize, GameUtil givenConfig, Random givenRandom)
  {
	  ...
	  r = givenRandom;
	  g = givenConfig;
	  grid = g.initializeNewGrid(size, r);
	  tempGrid = g.copyGrid(grid);
	  nextVal = g.generateRandomTileValue(givenRandom);
	  pending = false;
  }
  ...
  public int getScore()
  {
	  return g.calculateTotalScore(grid);
  }
  ...
  public int[] copyRowOrColumn(int rowOrColumn, Direction dir)
  {
	  ...
	  for (int i = size - 1; i >= 0; i--)
	  {
		  ...
		  {
			  arr[size - 1 - i] = grid[i][rowOrColumn];
		  }
		  
		  if(dir == Direction.RIGHT)
		  {
			  arr[size - 1 - i] = grid[rowOrColumn][i];
		  }
	  }
	  return arr;
  }
  ...
  public void updateRowOrColumn(int[] arr, int rowOrColumn, Direction dir)
  {
	  ...
	  for (int i = 0; i < arr.length; i++)
	  {
		  if(dir == Direction.UP)
		  {
			  grid[i][rowOrColumn] = arr2[i];
		  }
		  
		  if(dir == Direction.LEFT)
		  {
			  grid[rowOrColumn][i] = arr2[i];
		  }
	  }
	  ...
	  for (int i = arr.length - 1; i >= 0; i--)
	  {
		  if(dir == Direction.DOWN)
		  {
			  grid[i][rowOrColumn] = arr2[arr.length - 1 - i];
		  }
		  
		  if(dir == Direction.RIGHT)
		  {
			  grid[rowOrColumn][i] = arr2[arr.length - 1 - i];
		  }
	  }
  }
  ...
  public ArrayList<Move> shiftGrid(Direction dir)
  {
	  ...
	  for (int i = 0; i < size; i++)
	  {
		  arr = copyRowOrColumn(i, dir);
		  tempList = g.shiftArray(arr);
//		  System.out.println(list.toString());
		  for (int j = 0; j < tempList.size(); j++)
		  {
			  tempList.get(j).setDirection(i, dir);
		  }
		  list.addAll(tempList);
		  updateRowOrColumn(arr, i, dir);
	  }
	  nextVal = g.generateRandomTileValue(r);
	  pending = true;
	  return list;
  }
  ...
  public TilePosition newTile()
  {
	  if (pending)
	  {
		  TilePosition t = g.generateRandomTilePosition(grid, r, d);
		  if(t != null)
		  {
			  t.setValue(nextVal);
			  //setCell(t.getRow(), t.getCol(), getNextTileValue());
